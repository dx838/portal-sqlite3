    ...
    verifyAuthorization(req)
        .then(userInfo => {
            let sqlBase = `
                           ...
                           LEFT JOIN users users_init ON wubi_words.uid = users_init.uid
                           LEFT JOIN users users_modify ON wubi_words.uid = users_modify.uid
            ...
            // approved 字段在 SQLite 表中不存在，注释掉该条件
            ...
            Promise
                ...
                .then(([dataList, dataSum]) => {
                    // 不再使用 unicodeDecode 处理数据，避免 emoji 字符显示为乱码
                    updateUserLastLoginTime(userInfo.uid)
...
router.post('/check-exist', (req, res) => {
    verifyAuthorization(req)
        .then(userInfo => {
            let sqlArray = []
            sqlArray.push(`
                select * from ${CURRENT_TABLE} where word like '%${req.body.word}%' and code like '${req.body.code}%' limit 5`
            )
            ...
            sqlArray.push(
                `
                    ...
                    ${CURRENT_TABLE}(
                        ...
                        comment, uid, category_id 
                        )
                    VALUES(
                        '${req.body.word}','${req.body.code}','${req.body.priority || 0}','${req.body.up || 0}','${req.body.down || 0}',
...
router.post('/add-batch', (req, res) => {
    // 1. 验证用户信息是否正确
    logInfo('===== POST /portal/wubi/word/add-batch 参数 =====')
    logInfo('req.body:', req.body)
    logInfo('priority:', req.body.words?.map(word => word.priority))
    logInfo('category_id:', req.body.category_id)
    logInfo('========================================')
                ...
                .then(results => {
                    ...
                    if (insertWords.length === 0){
                        ...
                        let sqlArray = insertWords.map(word => {
                            let timeNow = dateFormatter(new Date())
                            return `INSERT into ${CURRENT_TABLE}(word, code, priority, up, down,
                                                          date_create, date_modify, comment, uid, category_id)
                                VALUES ('${word.word}', '${word.code}', '${word.priority || 0}', '${word.up || 0}', '${word.down || 0}',
