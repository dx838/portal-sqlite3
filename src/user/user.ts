...
router.post('/login', (req, res) => {
    try {
        ...
        // 验证必填字段
        if (!email || !password) {
            return res.send(new ResponseError('', '邮箱和密码不能为空'));
        }
        
        let sqlArray = [];
        sqlArray.push(`select * from ${CURRENT_TABLE} where email = '${email}'`);

        getDataFromDB(DB_NAME, sqlArray, true)
            .then(data => {
                if (data) {
                    // 使用 Promise 形式的 bcrypt.compare
                    bcrypt.compare(password, data.password)
                        .then(isPasswordMatch => {
                            if (isPasswordMatch) {
                                updateUserLastLoginTime(data.uid);
                        ...
                        .catch(err => {
                            ...
                            res.send(new ResponseError('', '密码验证失败'));
                        });
                } else {
            ...
            .catch(err => {
                ...
                res.send(new ResponseError('', '数据库查询失败'));
            });
    } catch (err: any) {
        console.error('登录接口异常:', err);
        res.send(new ResponseError('', '服务器内部错误'));
    }
})
...
//                 .catch(queryErr => {
//                     // 回滚事务
//                     getDataFromDB(DB_NAME, ['ROLLBACK;'])
//                         .then(() => {
//                             res.send(new ResponseError(queryErr, '事务执行失败，已回滚'))
//                         })
//                         .catch(rollbackErr => {
//                             res.send(new ResponseError(rollbackErr, '事务执行失败，回滚也失败'))
//                         })
//                 })
//         })
//         .catch(errInfo => {
