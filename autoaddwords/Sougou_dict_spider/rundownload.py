...
def rundownload(Categories,SavePath,txtSavePath,jsonfilepath='') -> None:
    # 缺少参数直接退出
    ...
    for category in Categories:
        ...
        if category_id in allcategory:
            allcategory[category_id].append(category_name)
        else:
            allcategory[category_id] = [category_name]
    #
    # print(f"allcategory: {allcategory}")
    # 大类分类
    for category_id,category_names in allcategory.items():
        # 创建保存路径 直接放一个目录下面
        ...
        for page in range(1, pages + 1):
            ...
            for keyDownload, urlDownload in downloadUrls.items():
                # 只下载指定的文件
                if keyDownload in allcategory[category_id]:
                    ...
                    # 下载完指定的后就跳出，好下载下一个
                if not (set(allcategory[category_id]) - set(already_download)) :  # 下载完指定的词库后退出
                    break
                # else:
...
if __name__ == '__main__':
    ...
    rundownload(Categories,SavePath,txtSavePath)
